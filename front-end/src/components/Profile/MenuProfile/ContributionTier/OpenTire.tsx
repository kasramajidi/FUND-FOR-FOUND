...
import usePostTire from "./usePostTire";
...
export default function OpenTire({ onClose, brandId }) {
  const [state, dispatch] = useReducer(reducer, initialState);
  const postTire = usePostTire();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      postTire.mutate(dataToSend, {
        ...
        onError: (error) => {
          setIsUploading(false);
          console.error("Error in postTire mutation:", error);
          alert("Failed to save tire: " + error.message);
        },
      });
    } catch {
      setIsUploading(false);
        ...
        <div className="flex-1 p-8 flex flex-col shadow-2xl">
          ...
          <div className="flex gap-4 mt-auto">
            ...
            <button
              ...
              onClick={handleSave}
              disabled={postTire.isPending || isUploading}
            >
              {postTire.isPending || isUploading ? "Saving..." : "Save"}
            </button>
          </div>
        </div>
        {/* پیش‌نمایش سمت راست */}
