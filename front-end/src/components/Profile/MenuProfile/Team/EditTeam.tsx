...
export default function EditTeam({ team, onClose }) {
  ...
  const handleRemove = async () => {
    try {
      await removeTeam.mutateAsync({ id: team.documentId, name: team.name });
      onClose();
    } catch (error) {
  ...
  return (
    <>
          ...
          <div className="flex-1 bg-white rounded-lg p-3 sm:p-4 md:p-6 shadow-md border border-[rgba(231,231,231,1)]">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="flex flex-col">
                ...
                <span className="w-full border text-[rgba(149,149,149,1)] cursor-no-drop border-[#8d75f7] rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#8d75f7]">
                  {team.role}
                </span>
              </div>
              <div>
              ...
              <div className="flex w-full justify-between gap-3 mt-6">
                <div className="flex gap-3 items-center">
                  <button
                    onClick={onClose}
                    type="button"
                    className="px-3 py-1.5 text-sm sm:px-4 sm:py-2 sm:text-base lg:px-6 lg:py-2 rounded border border-[rgba(170,153,236,1)] cursor-pointer text-[rgba(100,79,193,1)] bg-[rgba(237,233,254,1)] hover:bg-[#f3f0ff]"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-3 py-1.5 text-sm sm:px-4 sm:py-2 sm:text-base lg:px-6 lg:py-2 rounded bg-[rgba(100,79,193,1)] cursor-pointer text-white font-semibold hover:bg-[rgba(100,79,193,0.8)]"
                  >
                    save
                  </button>
                </div>
                <button
                  ...
                  className={`px-3 py-1.5 text-sm sm:px-4 sm:py-2 sm:text-base lg:px-6 lg:py-2 rounded flex items-center gap-2 font-semibold
                    ${
                      team.role === "Admin"
                        ? "bg-[rgba(253,216,216,1)] text-white cursor-not-allowed"
                        : "bg-red-600 hover:bg-red-700 cursor-pointer text-white"
                    }
                  `}
                  title={
                    team.role === "Admin" ? "Admin cannot be removed" : "Remove"
                  }
                >
                  <FaRegTrashAlt />
                  Remove
            ...
            <div className="w-full flex flex-col items-center border rounded-lg border-[rgba(199,198,198,1)]">
              ...
              <div className="text-center">
                ...
                <div className="mt-3 text-gray-500 text-sm w-full max-w-full max-h-24 overflow-y-auto break-words whitespace-pre-line break-all">
                  {state.description || team.discription}
      ...
      {removeTeam.showSuccess && removeTeam.removedTeam && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#575757]/70 backdrop-blur-sm">
