...
export default function BandOrOrganization() {
  ...
  if (isLoading) {
    return (
      <div className="text-center py-8">
        Loading band or organization data...
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="text-center py-8 text-red-500">
        Error: {error?.message || "Failed to load band or organization data"}
      </div>
    );
  }
  ...
  if (!brands || brands.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        No band or organization data found.
      </div>
    );
  }
